...
- New `channelOk` gate: point 4 must sit at least 0.5 ATR off the 1-3 line (`p4WidthAtr`) and the completed point-4-to-point-5 leg must cross at least 60% of the remaining channel width (`traversal45`) — previously `channelOk` (point-4-in-channel only) was scored (8 pts) but never gated
- New `legPurityOk` gate (`maxLegPurityRatio`, default 0.50): rejects a combinatorial point selection if any skipped intermediate swing deviates from its enclosing leg's straight path by more than half that leg's own amplitude — stops the search from picking a "clean" 5-point chain that ignores a near-equal countermove
- New `minRR` gate (default 1.5): risk/reward is now computed from point-5-to-invalidation-level risk vs. the EPA-now reward; `rewardOk` requires direction, max target distance, AND minimum R-multiple together, instead of direction+distance alone
- `invalidationAtr` (default 0.5 ATR) is now an input instead of hardcoded, and the level itself factors in the live 1-3 line (`min/max(y5, line13At5) ± buffer`), not just point 5; a new `dynamicInvalidationBars` window (default 10 bars) keeps the level following the 1-3 line for that many bars after point 5 before it freezes
- Score rebuilt as 9 continuous (not binary pass/fail) criteria summing to 100: structure 20, contraction 15, channel 10, apex/convergence 10, P5-zone 15, risk/reward 15, timing 5, symmetry 5, leg purity 5
- `valid` now additionally gates `contractionOk` and `channelOk`, on top of the existing `structureOk`/`boundaryHoldOk`/`barsInsideOk`/`p5ZoneOk`/`etaOk` vetoes and the new `legPurityOk` — a pattern can no longer validate on score alone with a broken channel or a non-contracting wedge
- Pattern lifecycle replaced: each tracked pattern now runs a state machine (`ACTIVE` → `PARTIAL_SUCCESS` → `TARGET_HIT` / `EXPIRED` / `INVALIDATED`) instead of only tracking invalidation. The EPA target is now live (recomputed from line 1-4 every bar, not frozen at the apex projection); a 50%-to-target level fires a one-time "Partial" marker; `expiryBufferBars` (default 10) marks a pattern `EXPIRED` once price hasn't hit the target within that many bars past its computed apex bar
- Three new alerts: `WWSP · PARTIAL SUCCESS`, `WWSP · EPA TARGET HIT`, `WWSP · EXPIRED`, alongside the existing bull/bear/invalidated alerts
- Fixed `localBestForI5` preferring a higher-scoring-but-invalid candidate over a valid lower-scoring one for the same point-5 anchor: it now tracks the raw highest-score candidate separately and only falls back to it when no valid candidate exists for that anchor
- Fixed the global `best` candidate updating before the per-`i5` bar-containment check (`f_bars_inside_metrics`, renamed from `f_bars_inside_ok`) instead of after — the dashboard could show a pre-validation winner the containment check would go on to reject
- `patternAtr` (ATR sampled at the pattern's own point-5 bar) replaces the live `atr` in every ATR-normalized threshold and score, so a pattern's own vola context at formation time — not the current bar's — governs its gates
- Dashboard gained an "EPA Current" row (live target) alongside "EPA @ ETA" (apex projection); debug table extended with rows for leg purity, swing/bar boundary violation distance, channel widths, and P5 offset; candidate marker now also shows a "WATCH" label with a list of which gate(s) are blocking it
- `Invalidierte/ersetzte Patterns behalten` renamed to `Abgeschlossene/ersetzte Patterns behalten` to reflect that patterns can now also complete via `TARGET_HIT`/`EXPIRED`, not just invalidation/replacement/eviction
