...
| [market_tradability_engine](indicators/market_structure/market_tradability_engine/) | 1.3.2 / 2.2.1 | Draft | ★★★★☆ | ★★★☆☆ | ☆☆☆☆☆ | Markt-Qualitäts-/No-Trade-Filter statt Signalgenerator. v1 (stabiler, unversionierter Dateiname): Tradability/Balance/Compression/Direction-Scores treiben eine 8-Zustands-Maschine (No Trade/Balance/Compression/Ready/Breakout/Trend/Aftermath/Exhaustion) mit Disagreement-Streak-Confirmation/Mindestdauer-Hysterese; ADX×Efficiency-Gate, Dual-Window-Efficiency (fast/slow) für Aftermath, Breakout-Acceptance-Bars, Ready-Energy-Building-Gate, Compression-Energy-Komponente, Post-Breakout-Balance-Sperre; Ratio-Scores jetzt Percentile-Rank-basiert (`ta.percentrank`) statt fixer Konstanten für Timeframe-/Instrument-Robustheit; Dashboard mit Kandidat-Zeile; Pine-Log-Kalibrierung. v2.2.1 (separate Datei, Beta, parallel zu v1, unkalibriert): reduziert die vier v1-Scores auf drei fundamentale Eigenschaften — Structure/Energy/Acceptance — aus denen alle 8 Zustände abgeleitet werden; gleiche State-Machine-Mechanik wie v1; gleiche Percentile-Rank-Normalisierung; separates Reactivity-Preset (Early/Balanced/Conservative/Custom) für Trigger-Empfindlichkeit unabhängig vom Längen-Preset; Baseline-Type-Auswahl (EMA/SMA/RMA/WMA/SuperSmoother/T3/KAMA/JMA Approx, "Advanced family"-Muster); MTE2-Kalibrierungslog für Side-by-side-Vergleich mit v1; ungetestet |
...
| [triangle_compression_scanner_pro](indicators/market_structure/triangle_compression_scanner_pro/) | 1.13 | Draft | ★★★★☆ | ★★★☆☆ | ☆☆☆☆☆ | Gleitendes-Fenster-Dreieck-Scanner (symmetrisch/ascending/descending): Begrenzungslinien als Best-Boundary-Fit durch echte Touch-Punkte (nicht Least-Squares) über ein gleitendes Swing-Fenster, Score aus Struktur/Kompression(≥25%+monoton)/Touch-Qualität/Linien-Qualität/Dauer, Mehrfach-Pattern-Tracking + Overlap-Ersetzung/Eviction, Live-Breakout-Tracking (beide Richtungen) + Alert; v1.4–1.12: Default nur 1 gleichzeitiges Pattern, Linie friert nach 1× Fensterbreite ein statt endlos zu driften, Alter (× Fensterbreite) statt ATR-Drift als primärer Cleanup, `barsInsideOk`-Hard-Veto prüft jeden Bar (nicht nur Swings) gegen beide Grenzen; v1.13: Near-Miss-Kandidatenvorschau (`showCandidate`/`candidateThreshold`) mit gestrichelten Preview-Linien für das aktuell beste Shape zwischen Candidate- und Min-Score + "Triangle Candidate"-Watch-Marker (portiert aus Wolfe/Broadening Wedge Scanner Pro); Kandidat/Debug/Dashboard nach Wolfe-Vorbild, eigener Code (keine geteilte Lib); Schwester-Indikator zu Broadening Wedge Scanner Pro (Kompression statt Expansion); noch nicht am Chart validiert |
