...
| [`structure_break_risk.pine`](indicators/market_structure/structure_break_risk/structure_break_risk.pine) | Chart overlay of how close the prevailing trend's structure is to breaking, and against which level — a break-level line, a shaded risk zone and word event labels in price, with the 0–100 risk magnitude in a compact, mobile-friendly info label; symmetric top-/bottom-break scoring from five weighted sensors (near level, confirmed BOS, failed breakout, structure erosion, pivot divergence); companion to Trend Persistence Score |
